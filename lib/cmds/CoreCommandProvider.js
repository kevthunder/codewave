...
const noExecute = function (instance) {
  var reg
...
const quoteCarret = function (instance) {
  return instance.content.replace(/\|/g, '||')
}

const execParent = function (instance) {
  var res
...
getContent = function (instance) {
  const affixesEmpty = instance.getParam(['affixes_empty'], false)
  const prefix = instance.getParam(['prefix'], '')
  const suffix = instance.getParam(['suffix'], '')
...
removeCommand = function (instance) {
  return Promise.resolve().then(() => {
    const name = instance.getParam([0, 'cmd'])
    const storage = Command.storage
    ...
    if (name != null) {
      return Promise.resolve().then(() => {
        ...
        const cmd = instance.context.getParentOrRoot().getCmd(name)
...
CloseCmd = class CloseCmd extends BaseCommand {
  ...
  execute () {
    const prefix = this.helper.prefix = this.instance.getParam(['prefix'], '')
    const suffix = this.helper.suffix = this.instance.getParam(['suffix'], '')
    let box = this.helper.getBoxForPos(this.instance.getPos())
    const requiredAffixes = this.instance.getParam(['required_affixes'], true)

    if (!requiredAffixes) {
      this.helper.prefix = this.helper.suffix = ''
      const box2 = this.helper.getBoxForPos(this.instance.getPos())
    ...
    if (box != null) {
      const depth = this.helper.getNestedLvl(this.instance.getPos().start)
...
TemplateCmd = class TemplateCmd extends BaseCommand {
  init () {
...
EmmetCmd = class EmmetCmd extends BaseCommand {
  ...
  result () {
    const emmet = this.getEmmet()

    if (emmet != null) {
      // emmet.require('./parser/abbreviation').expand('ul>li', {pastedContent:'lorem'})
      const res = emmet.expandAbbreviation(this.abbr, this.lang)
