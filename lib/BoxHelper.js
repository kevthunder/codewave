...
var BoxHelper = class BoxHelper {
  ...
  clone (text) {
    const opt = Object.keys(this.defaults).reduce((opt, key) => {
      opt[key] = this[key]
      return opt
    }, {})
  ...
  removeComment (text, options = {}) {
    if (text != null) {
      const ecl = StringHelper.escapeRegExp(this.context.wrapCommentLeft())
      const ecr = StringHelper.escapeRegExp(this.context.wrapCommentRight())
      const ed = StringHelper.escapeRegExp(this.deco)
      const flag = options.multiline ? 'gm' : ''
      const re1 = new RegExp(`^\\s*${ecl}(?:${ed})*\\s{0,${this.pad}}`, flag)
      const re2 = new RegExp(`\\s*(?:${ed})*${ecr}\\s*$`, flag)
