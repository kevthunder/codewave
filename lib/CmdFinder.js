...
const NamespaceHelper = require("./helpers/NamespaceHelper");
...
var CmdFinder = class CmdFinder {
  ...
  getNamesWithPaths() {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      name = ref[j];
      [space, rest] = NamespaceHelper.NamespaceHelper.splitFirst(name);
  ...
  applySpaceOnNames(namespace) {
    var rest, space;
    [space, rest] = NamespaceHelper.NamespaceHelper.splitFirst(namespace, true);
    return this.names.map(function (name) {
      var cur_rest, cur_space;
      [cur_space, cur_rest] = NamespaceHelper.NamespaceHelper.splitFirst(name);
  ...
  findPosibilities() {
    ...
    for (j = 0, len = ref3.length; j < len; j++) {
      nspc = ref3[j];
      [nspcName, rest] = NamespaceHelper.NamespaceHelper.splitFirst(nspc, true);
