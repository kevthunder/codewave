...
var CmdFinder = class CmdFinder {
  ...
  getNamesWithPaths() {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      name = ref[j];
      [space, rest] = _NamespaceHelper.NamespaceHelper.splitFirst(name);
  ...
  applySpaceOnNames(namespace) {
    var rest, space;
    [space, rest] = _NamespaceHelper.NamespaceHelper.splitFirst(namespace, true);
    return this.names.map(function (name) {
      var cur_rest, cur_space;
      [cur_space, cur_rest] = _NamespaceHelper.NamespaceHelper.splitFirst(name);
  ...
  findPosibilities() {
    ...
    for (j = 0, len = ref2.length; j < len; j++) {
      nspc = ref2[j];
      [nspcName, rest] = _NamespaceHelper.NamespaceHelper.splitFirst(nspc, true);
